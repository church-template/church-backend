...
test/.../domain/position/PositionApiTest.java           // @SpringBootTest + MockMvc + Testcontainers (인가·검증·CRUD·에러코드)
```
...
공통 헬퍼 — `normalizeName(String raw)`: `raw`가 null이거나 `trim()` 후 `isBlank()`면 `INVALID_INPUT_VALUE`, 아니면 trim된 값 반환(서비스 직접 호출 시 null이 NPE/500로 새지 않게 방어). name이 들어오는 모든 경로(create는 항상, update는 `name != null`일 때)가 이 헬퍼를 거쳐 trim·blank 거부·유니크 검사 대상값을 일관 산출한다.
...
**PositionApiTest** (`@SpringBootTest` + `MockMvc` + Testcontainers, 실 JWT): 공개/관리자 엔드포인트의 인가·검증·CRUD·에러코드를 풀스택으로 통합 검증한다. (설계 초안의 `@WebMvcTest` 분리 컨트롤러 테스트 + 별도 통합 테스트를, 보안 컨텍스트가 필요한 인가 검증과 실 Flyway·`validate` 정합 검증을 함께 다루도록 단일 `@SpringBootTest`+MockMvc 테스트로 통합 — 저장소 선례 `SecurityConfigPathRulesTest`와 동일.)

| # | 케이스 | 기대 |
|---|---|---|
| 17 | GET /api/positions 무인증 | 200, JSON 평배열, sort_order 순, `page` 봉투 없음 |
| 18 | POST sortOrder 누락 | 201, 빈 테이블 `10` → 다음 `20`(간격) |
| 19 | POST 무인증(익명) | 401 `INVALID_TOKEN` (G3 핸들러) |
| 20 | POST 권한 부족(다른 authority) | 403 `ACCESS_DENIED` |
| 21 | POST name 공백 | 400 `INVALID_INPUT_VALUE` |
| 22 | POST 중복 name | 409 `DUPLICATE_RESOURCE` |
| 23 | PATCH sortOrder만 | 200, name 유지·sortOrder 변경 |
| 24 | PATCH name 변경 | 200, 새 이름 DB 반영 |
| 25 | PATCH 없는 id | 404 `RESOURCE_NOT_FOUND` |
| 26 | DELETE | 204, 목록에서 제거 |
