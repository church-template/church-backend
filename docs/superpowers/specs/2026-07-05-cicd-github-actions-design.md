...
- SSH로 실행:
  ...
  IMAGE_NAME=ghcr.io/{owner/repo} IMAGE_TAG={version} \
    docker compose -f docker-compose.yml -f docker-compose.prod.yml pull backend
  IMAGE_NAME=ghcr.io/{owner/repo} IMAGE_TAG={version} \
    docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d
  timeout 180 bash -c 'until curl -fs http://localhost:8080/actuator/health >/dev/null; do sleep 5; done'
...
- 이미지명은 워크플로우가 `IMAGE_NAME`(=`ghcr.io/<owner>/<repo>`)으로 주입하고 오버레이 기본값은 표준 템플릿 이미지 — 레포를 복사한 교회에서도 워크플로우·오버레이 코드 수정 없이 자기 GHCR을 쓴다 (최종 리뷰 반영).
