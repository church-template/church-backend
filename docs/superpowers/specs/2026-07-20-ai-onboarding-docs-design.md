...
## 6. docker-compose.override.yml — 미추적 확인 (정정)

~~초안은 "gitignore 선언과 달리 추적 중"이라는 모순을 보고했으나~~ **구현 시 재검증 결과 오독이었다**
(ls-files 무출력 + check-ignore 출력을 하나로 잘못 읽음). override는 처음부터 git에 없고,
`.gitignore:53` 선언과 실제가 일치한다. 조치 불요.

유효한 귀결만 남는다: **fresh clone에는 override가 없으므로** setup-dev.md가
"override 생성" 스텝(전체 YAML 블록)을 반드시 포함해야 한다 — dev 프로필·시드·호스트 포트
(5432·6379) 노출이 전부 이 파일에 달려 있다.
