...
--   - 앱이 만드는 실데이터(회원 가입, 부트스트랩 SUPER_ADMIN, 관리자 콘텐츠)는 말미 시퀀스 예약 덕에
--     10000 이상 id를 받으므로, 9000번대 고정 id는 그들과 절대 충돌하지 않는다.
--   - "id 9000~9999 = 개발 시드"로 한눈에 구분된다. 정리하려면 id 9000~9999 행만 지우면 된다
--     (id>=9000 삭제는 10000+ 실데이터까지 지우므로 금지).
